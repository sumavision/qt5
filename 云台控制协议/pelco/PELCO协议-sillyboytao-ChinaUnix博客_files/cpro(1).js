        ...
        sync: function () {
            ...
            if (isNeedSend || isNeedSendBid) {
                ...
                if (cproId && baiduId) {
                    ...
                    log.sendByIframe("//s.cpro.baidu.com/s.htm?cproid=" + urlCproId + "&t=" + currentTimeStamp);
                }
                if (cproId) {
                    log.sendByIframe("//weibo.com/aj/static/sync.html?t=" + currentTimeStamp);
                    log.sendByIframe("//pos.baidu.com/sync_pos.htm?cproid="+encodeURIComponent(cproId)+"&t=" + currentTimeStamp);
                    var urlCproId = cproId;
                    var cproIdIndex = cproId.indexOf(":");
                    if (cproIdIndex && cproIdIndex > 0) {
                        urlCproId = cproId.substring(0, cproIdIndex);
        ...
        */
        paint: function (option) {
            ...
            if (fullConfig.displayType === "inlay" && fullConfig.stuffType === "linkunit" && fullConfig.urlReplace && fullConfig.urlReplace !== "default") {
                for (var i = 0, count = data.length; i < count; i++) {
                    data[i].curl = data[i].curl.replace("//cpro.baidu.com/cpro/ui/uijs.php?", decodeURIComponent(fullConfig.urlReplace));
                }
            }
        ...
        */
        paint: function (option) {
            ...
            if (slotData.displayType === "inlay" && slotData.stuffType === "linkunit" && slotData.urlReplace && slotData.urlReplace !== "default") {
                for (var i = 0, count = data.length; i < count; i++) {
                    data[i].curl = data[i].curl.replace("//cpro.baidu.com/cpro/ui/uijs.php?", decodeURIComponent(slotData.urlReplace));
                }
            }
    ...
    return {
        ...
        */
        drawLogo: function(option){       
            ...
            if(isGongyi){
                logoDom.href="//gongyi.baidu.com/";
                logoDom.title="\u767e\u5ea6\u516c\u76ca";                
            }
            else{
                logoDom.href="//wangmeng.baidu.com/";
        ...
        */
        paint: function (option) {
            ...
            //<<a id="logo" class="logo" title="推广用户：www.tczlq.com" href="//wangmeng.baidu.com/" target="_blank">&nbsp;</a>
            //添加推广logo
